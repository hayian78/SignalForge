...
def _invoke(db_path: Path, vault_dir: Path, *extra: str) -> Result:
...
def test_digest_writes_the_expected_file(db_path: Path, vault_dir: Path) -> None:
    _seed(db_path)

    result = _invoke(db_path, vault_dir)
...
def test_digest_run_is_recorded_as_ok(db_path: Path, vault_dir: Path) -> None:
    _seed(db_path)

    assert _invoke(db_path, vault_dir).exit_code == 0
...
def test_digest_with_no_scored_items_still_succeeds(db_path: Path, vault_dir: Path) -> None:
    result = _invoke(db_path, vault_dir)
...
def test_running_digest_twice_overwrites_the_same_file(db_path: Path, vault_dir: Path) -> None:
    """The idempotency gate for `digest`, end to end through the CLI (CLAUDE.md §3)."""
    _seed(db_path)

    first = _invoke(db_path, vault_dir)
    assert first.exit_code == 0, first.output
    path = vault_dir / "daily" / "2026-07-16.md"
    first_content = path.read_text(encoding="utf-8")

    second = _invoke(db_path, vault_dir)
...
def test_dry_run_writes_nothing(db_path: Path, vault_dir: Path) -> None:
    _seed(db_path)

    result = _invoke(db_path, vault_dir, "--dry-run")
...
def test_dry_run_still_records_a_run(db_path: Path, vault_dir: Path) -> None:
    _seed(db_path)

    assert _invoke(db_path, vault_dir, "--dry-run").exit_code == 0

    run = _runs(db_path)[-1]
    assert run["status"] == "ok"


def test_digest_defaults_to_todays_date_when_omitted(db_path: Path, vault_dir: Path) -> None:
    ...
    result = runner.invoke(
        app,
        ["--log-level", "WARNING", "digest", "--db", str(db_path), "--vault-dir", str(vault_dir)],
    )
...
def test_digest_run_is_recorded_as_failed_when_rendering_blows_up(
    db_path: Path, vault_dir: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _seed(db_path)
