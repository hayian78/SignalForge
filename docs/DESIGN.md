...
| Podcast episode script (§13.3) | `claude-opus-5` | At most two requests per episode — the first attempt, and **at most one retry of either mode, never both** (`synth/podcast.py::build_script`, enforced by its `retry_spent` guard): `retry_mode="shorter"` if a complete script ran over `PODCAST_MAX_SCRIPT_CHARS`, or `retry_mode="unfinished"` if the first attempt was cut off at its output ceiling and returned nothing usable. The two fire on disjoint conditions (a valid script vs. no script), which is what keeps "at most two" true now that there are two retry paths. Prompt-cached stable prefix (show-format brief + interests/taxonomy, `build_podcast_stable_prefix`), the day's top-`podcast_top_n` items (titles, summaries, and full deep-read content — unlike triage, NEVER rule 9's sibling exemption) after the breakpoint | 1 call/day (2 on a long or a cut-off episode); item count capped by `PODCAST_MAX_ITEMS`, output by `PODCAST_MAX_TOKENS` (12,288) for the first call and `PODCAST_RETRY_MAX_TOKENS` (4,096) / `PODCAST_TRUNCATION_RETRY_MAX_TOKENS` (8,192) for whichever retry fires |
...
**Cost estimate.** Per line: triage ≈ 150 items/day × ~700 tokens ≈ 3.2M input tokens/month on Haiku via Batches ≈ **$1.60**; weekly brief ≈ 4.33 × (~14k in ≈ $0.07 + ~7k out including adaptive thinking ≈ $0.18) ≈ **$1.10** (replacing an earlier `4 × (80k in / 8k out) ≈ $2.40` line that was wrong in both terms — real input is ~14 kB, not 80k tokens, and it double-counted the unbuilt impact engine; the reduction is stated rather than pocketed. First measured run 2026-08-08 (`runs` id 151): 3,300 in / 2,143 out = $0.07, so this line is conservative, and it stays unmeasured until four real Sundays exist); deep reads and monthly report ≈ **$3.00**; curation scout ≈ 4.33 × (12 searches × ~28k in/search, the average of the three real runs measured so far ≈ $1.69 + ~7.6k out ≈ $0.19 + 12 searches ≈ $0.12) ≈ **$8.65**; podcast script ≈ 30 × (~15k in / ~3k out on `claude-opus-5`) ≈ **$4.50**; podcast TTS ≈ 30 episodes × ~8k spoken characters (a nine-to-twelve-minute show) on the shipped `hexgrad/kokoro-82m` rate ($0.62/1M chars) ≈ **$0.15**. **Itemized total ≈ $19.00/month**, against a **$5–10 target** and a **$50 alarm**.

**The alarm moved from $30 to $50 on 2026-08-16, by operator decision.** The trigger was a measured
run-rate, not a paper estimate: Aug 1–15 in `runs` extrapolated to **≈ $29.30/month** — scout $15.68,
podcast $11.95, weekly $1.13, triage $0.46, TTS $0.10 — leaving roughly seventy cents of headroom and
negative headroom the moment the podcast cron stopped skipping days. Two reductions were costed and
offered: reverting `curation.max_searches_per_run` from 12 to 6 (**−$10.40**) and stopping the
podcast's token-limit retry, which about 40% of episodes were paying for (**−$5.00**). Together they
would have landed the pipeline near $14.

The operator declined both and raised the band instead, judging the current scout depth and podcast
length worth their price. That is a legitimate call and it is recorded here rather than absorbed
quietly, because the alarm is the only number standing between this pipeline and unnoticed spend:
**the $5–10 target is unchanged and is still what the design aims at.** $50 is the line at which
something is wrong, not a new budget to grow into. The two reductions above remain costed and
available; nothing about them expired.
...
Two things that estimate is not. It is not measured: the only measured figures to date are **≈ $0.40/month actual** on triage (July 2026: 23 `score` runs, 0.37M input / 0.09M output on Haiku) and **three real scout runs** on 2026-07-30/31 (~$1.20 at 6 searches, ~$1.20 at 6 again, ~$1.02 at 7), because everything else prices a component that does not exist yet or has run too few times to average — the podcast script and TTS lines above are estimates in the same unmeasured category, priced against the shipped config's `podcast_top_n`/`hexgrad/kokoro-82m` defaults rather than an observed month of episodes. And the band is no longer mid-range: the scout alone, raised deliberately to 12 searches after the operator judged the extra research depth worth it, already accounted for over half the itemized total before the podcast landed, and the podcast's own two lines have pushed the **whole pipeline further past its own $5–10 target on paper** — still under the $50 alarm on these realistic volumes, but the next new LLM or TTS consumer, or a scout re-tune in the other direction, needs the band revisited rather than absorbed.

**The alarm now covers TTS characters, not just LLM tokens.** `signalforge status`'s month-to-date readout prices `runs.tts_characters` alongside `runs.llm_*_tokens` (Stage 6, DESIGN §13.3) — the TTS line above is not invisible to the alarm the way the search-tool dollar figure still is (next paragraph). Separately from the realistic estimate above, the **worst-case hard ceilings** now number four: `llm.SCOUT_MONTHLY_CEILING_USD` = **$13.00**, `llm.PODCAST_MONTHLY_CEILING_USD` = **$29.00** (raised from $23.00 on 2026-08-18), `deliver.tts.TTS_MONTHLY_CEILING_USD` ≈ **$43.40**, and `llm.WEEKLY_MONTHLY_CEILING_USD` = **$3.50** — summing to **$88.90**, still past $50 on their own. That sum is not a forecast: each ceiling is a hard cap priced at the most expensive plausible per-unit rate a config change could reach (every item field at its byte cap, a switch to the priciest listed TTS model) and enforced by code, the same discipline `SCOUT_MONTHLY_CEILING_USD` uses (§7.1). Recorded here rather than silently narrowed to fit under $30 by shrinking the show — see `PODCAST_MONTHLY_CEILING_USD`'s own docstring, which named this paragraph as its resolution.

**The podcast ceiling's sixth move, 2026-08-18, is the first not caused by a review finding the arithmetic wrong.** The arithmetic was right; the output ceiling it priced was too tight to finish a script under. Two episodes (2026-08-15, 2026-08-17) were lost inside four days to `PODCAST_MAX_TOKENS` truncation — both runs recorded *exactly* 8192 output tokens, returned truncated JSON that failed schema validation, and closed as `status="ok"` with an empty `errors` column, so the reports-are-the-monitoring-channel rule (§7) never fired. Two lessons, both structural rather than numeric: **a cost ceiling that the work cannot complete under is not a saving, it is a silent failure with a bill attached** (each lost episode still cost a full-ceiling Opus call); and **a failure that produces no exception still has to reach `runs.errors`** — `build_script` deliberately never raises once a call is billed, and its own docstring had recorded threading those outcomes into `runs.errors` as a later stage's job that never happened. The fix raises the ceiling to 12,288, adds a distinct truncation retry with enough room to succeed (`PODCAST_TRUNCATION_RETRY_MAX_TOKENS`), and records the reason — with the at-most-one-retry-per-run invariant the ceiling is priced on preserved by an explicit guard, since there are now two retry paths where there was one.
...
- **A new spend category outside `llm.py`'s token accounting.** TTS bills per
  ...
  §8's cost table and its honest note on what this does to the cost alarm.
